...
/** @type React.MutableRefObject<HTMLInputElement> */
  ...
  let markers = [];
  let routePath = new window.google.maps.Polyline({
    path: [],
    geodesic: true,
    strokeColor: "#33AAEE",
    strokeOpacity: 0.8,
  ...
  // var marker_org = new window.google.maps.Marker({label:{text:"A",color:"white"}});
  // var marker_dst = new window.google.maps.Marker({label:{text:"B",color:"white"}});
  ...
  async function calculateRoute() {
    ...
    // eslint-disable-next-line no-undef
    var route = latlng.map(x=>({lat:x[0],lng:x[1]}));
    route.splice(0,0,org);
    route.push(dst);
    ...
    // setDistance(distance2str(distance))
